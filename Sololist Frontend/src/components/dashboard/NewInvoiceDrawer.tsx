/**
 * NewInvoiceDrawer — slide-in sheet to draft & send an invoice.
 * Fixed: SheetTrigger now uses the Base UI "render" pattern.
 */
"use client";
...
export function NewInvoiceDrawer({ client, onInvoiceCreated, trigger }: NewInvoiceDrawerProps) {
  ...
  const nextInvoiceNum = `#${Math.floor(Math.random() * 900) + 100}`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(`Invoice ${formData.invoice_number} created for ${client.name}`);
      setOpen(false);
      onInvoiceCreated?.();
    } catch (error) {
      console.error(error);
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent className="sm:max-w-[480px] bg-background-primary/95 backdrop-blur border-l border-border-tertiary">
        <SheetHeader className="mb-8">
          ...
          <SheetDescription className="text-text-secondary text-[14px]">
            ...
            <span className="font-bold text-text-primary">{client.company}</span>.
          </SheetDescription>
        </SheetHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="bg-solo-blue/5 rounded-[16px] p-5 border border-solo-blue/10">
            ...
            <p className="text-[12px] font-medium text-text-secondary leading-relaxed">
              This amount aligns with your {client.project} contract terms. AI suggests sending now
              to avoid weekend payment delays.
            </p>
          </div>
